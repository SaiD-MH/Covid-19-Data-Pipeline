...
from src.etl.transform import (drop_columns , drop_null_values , drop_negative_values , format_locations , standardize_null_values )
@pytest.fixture
def dataframe():
    return pd.DataFrame({
        "col1" : ['1','2','3'],
...
(pd.DataFrame({'c1' : [1,1,1] , 'c2' : [-2,2,2] , 'c3' : [3,3,-3] }) , pd.DataFrame({'c1' : [1] , 'c2' : [2] , 'c3' : [3] }) ) , # Minx Pos and Neg
(pd.DataFrame({'c1' : [1,1,1] , 'c2' : [2,2,2] , 'c3' : [3,3,3] }) , pd.DataFrame({'c1' : [1,1,1] , 'c2' : [2,2,2] , 'c3' : [3,3,3] }) ) , # Postive Only
(pd.DataFrame({'c1' : [-1,-1,-1] , 'c2' : [-2,-2,-2] , 'c3' : [-3,-3,-3] }) , pd.DataFrame({'c1' : [] , 'c2' : [] , 'c3' : [] }) )   # Neg only
])
...
@pytest.mark.parametrize("df , expected" , [
(pd.DataFrame({'c1' : ['dATa'] , 'c2' : ["DATA"] , 'c3' : ['data'] }) , pd.DataFrame({'c1' : ['Data'] , 'c2' : ['Data'] , 'c3' : ['Data'] }) ) , 
(pd.DataFrame({'c1' : ['Data,data'] , 'c2' : ['  DatA   '] , 'c3' : ['data data   '] }) , pd.DataFrame({'c1' : ['Data,Data'] , 'c2' : ['Data'] , 'c3' : ['Data Data'] }) ) ,
])
...
@pytest.mark.parametrize("df , expected" , [
(pd.DataFrame({'c1' : ['data' ,'None'] , 'c2' : ["N/A" , "null"]}) , pd.DataFrame({'c1' : ['data' ,'None'] , 'c2' : ['None' , 'None']}) ) , 
(pd.DataFrame({'c1' : ['Unknown' ,''] , 'c2' : ['value' ,'None'] }) ,pd.DataFrame({'c1' : ['None' ,'None'] , 'c2' : ['value','None'] })  ) ,
])

...
def fill_date_dim_table(
    cleansed_data: pd.DataFrame, db_connection: DatabaseConnection
) -> pd.DataFrame:
    """
    ...
    date_exists = db_connection.read_dataframe_from_db(
        f"SELECT date_key from gold.date_dim where date_key = {date_key} "
    )
...
def fill_region_dim(
    ...
    new_regions = new_regions.drop_duplicates(
        subset=("province_state", "country_region")
    )
...
def load_all_dimension(dims_tables: list, db_connection: DatabaseConnection) -> list:
    ...
    try:
        ...
        for dim in dims_tables:
            total_loaded = db_connection.load_dataframe_into_db(
                dim["table"], "gold", dim["name"]
            )
...
def load_fact_table(fact_data: pd.DataFrame, db_connection: DatabaseConnection) -> int:
    ...
    try:
        return db_connection.load_dataframe_into_db(fact_data, "gold", "fact")

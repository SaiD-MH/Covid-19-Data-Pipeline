...
from src.etl.load import (fill_date_dim_table,fill_region_dim,fill_fact_table)
from pandas.testing import assert_frame_equal
...
def test_fill_date_dim_when_record_not_exists_in_db(mock_db_connection,cleansed_data , expected_date_record):
    ...
    assert_frame_equal(expected_date_record.reset_index(drop=True), result.reset_index(drop=True) , check_dtype=False)

def test_fill_date_dim_when_record_exists_in_db(mock_db_connection,cleansed_data , expected_date_record):
    ...
    result = fill_date_dim_table(cleansed_data , mock_db_connection)
    true_result = pd.DataFrame(columns=[
            'date_key', 'full_date', 'day_of_week', 'day_of_month',
            'day_name', 'week_of_year', 'month', 'month_name',
            'quarter', 'year', 'is_weekend'
        ])
    assert_frame_equal(true_result.reset_index(drop=True), result.reset_index(drop=True) , check_dtype=False)
...
def test_fill_fact_table(mock_db_connection , cleansed_data , expected_date_record , all_regions):
    ...
    expected = cleansed_data[['confirmed' , 'deaths','recovered' ,'active' , 'incident_rate','case_fatality_ratio']]

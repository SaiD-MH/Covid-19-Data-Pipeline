...
graph TB
    subgraph Source["📁 SOURCE SYSTEM"]
        CSV[("CSV Files<br/>DD-MM-YYYY.csv")]
    end

    subgraph Airflow["🔄 APACHE AIRFLOW ORCHESTRATION"]
        FS["FileSensor<br/>⏱️ Wait for File"]
        E["Extract Task<br/>📥 Python Operator"]
        T["Transform Task<br/>🔄 Python Operator"]
        L["Load Task<br/>📤 Python Operator"]
    end

    subgraph ETL["⚙️ ETL PIPELINE (Python)"]
        EXT["extract.py<br/>• Read CSV<br/>• Normalize columns<br/>• Add metadata"]
        TRF["transform.py<br/>• Data quality checks<br/>• Filter nulls/negatives<br/>• Standardize data"]
        LOD["load.py<br/>• Build dimensions<br/>• Create facts<br/>• Generate keys"]
    end

    subgraph Database["🗄️ POSTGRESQL DATABASE"]
        subgraph Bronze["🥉 BRONZE LAYER (Raw)"]
            B1[("bronze.covid<br/>Raw data + metadata")]
        end
        
        subgraph Silver["🥈 SILVER LAYER (Cleansed)"]
            S1[("silver.covid<br/>Quality-checked data")]
        end
        
        subgraph Gold["🥇 GOLD LAYER (Analytics)"]
            G1[("date_dim<br/>Date dimension")]
            G2[("region_dim<br/>Region dimension")]
            G3[("fact<br/>COVID metrics")]
        end
    end

    subgraph Testing["🧪 TESTING FRAMEWORK"]
        UT["Unit Tests<br/>✓ Mocked dependencies<br/>✓ Function-level tests"]
        IT["Integration Tests<br/>✓ Testcontainers<br/>✓ Real database<br/>✓ End-to-end validation"]
    end

    subgraph Infrastructure["🐳 INFRASTRUCTURE"]
        DC1["Docker Compose<br/>Dev Environment"]
        DC2["Docker Compose<br/>Test Environment"]
        DC3["Docker Compose<br/>Airflow Services"]
    end
    ...
    EXT -->|Load raw data| B1
    B1 -->|Read| TRF
    TRF -->|Load cleansed| S1
    S1 -->|Read| LOD
    LOD -->|Create dimensions| G1
    LOD -->|Create dimensions| G2
    LOD -->|Create facts| G3
    ...
    style UT fill:#d1ecf1
    style IT fill:#d1ecf1
    style DC1 fill:#e7f3ff
    style DC2 fill:#e7f3ff
    style DC3 fill:#e7f3ff

    classDef sourceStyle fill:#e1f5ff,stroke:#0077b6,stroke-width:2px
    classDef airflowStyle fill:#fff3cd,stroke:#ff9800,stroke-width:2px
    ...
    classDef testStyle fill:#d1ecf1,stroke:#0c5460,stroke-width:2px
    classDef infraStyle fill:#e7f3ff,stroke:#004085,stroke-width:2px

    class CSV sourceStyle
    class FS,E,T,L airflowStyle
    class B1 bronzeStyle
    class S1 silverStyle
    class G1,G2,G3 goldStyle
    class UT,IT testStyle
    class DC1,DC2,DC3 infraStyle
```
